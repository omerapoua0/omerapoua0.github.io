(() => {
  ...
  let mediaPaused=reduced.matches;
  const films=[...document.querySelectorAll('.ambient-film')];
  const button=document.querySelector('.film-control');
  const inView=new Set();
  const sync=()=>{films.forEach(v=>{if(mediaPaused||document.hidden||!inView.has(v))v.pause();else v.play().catch(()=>{})});if(button){button.textContent=mediaPaused?'Play background film':'Pause background film';button.setAttribute('aria-pressed',String(mediaPaused));}};
